...
#include "ui_shopdetail_buyer.h"

shopdetail_buyer::shopdetail_buyer(QString roamName,int projectID,QWidget *parent) :
    QWidget(parent),
    ui(new Ui::shopdetail_buyer)
{
    ui->setupUi(this);
    ...
    ui->buy_bt->setStyleSheet("QPushButton{color:white ; background-color:rgb(85, 170, 0)}");

    ui->no_lb->setStyleSheet("QLabel{color:red}");
    ...
    connect(ui->buy_bt,&QPushButton::clicked, this , &shopdetail_buyer::buy_push);
}

void shopdetail_buyer::changeView(int projectID)  //改变当前详情
{
    ui->no_lb->setVisible(false);
    nowID = projectID;
    sqilteOpreatorProjects Projectdb;
...
void shopdetail_buyer::buy_push()
{
    ui->no_lb->setVisible(false);
    sqilteOpreatorProjects Projectdb;
    Projectdb.openDb();
    ...
    if(nowProject.number == 0)
    {
        ui->no_lb->setVisible(true);
        return;
    }

    nowCar.projectInsertData(nowName , nowID);

}


shopdetail_buyer::~shopdetail_buyer()
{
    delete ui;
}
