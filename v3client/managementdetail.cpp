    ...
    ui(new Ui::managementDetail)
{
    ...
    connect(ui->prodiscount_line, &QLineEdit::textChanged, this , &managementDetail::enableButton);
...
void managementDetail::enableButton()  //编辑使能按钮
{
    bool useOK,passOK,passOK2,passOK3;
    useOK = !ui->proname_line->text().isEmpty();
    passOK = !ui->pronumber_line->text().isEmpty();
    passOK2 = !ui->proprice_line->text().isEmpty();
    passOK3 = !ui->prodiscount_line->text().isEmpty();
    ui->signup->setEnabled(useOK && passOK && passOK2 && passOK3);
    if( useOK && passOK && passOK2 && passOK3)
    {
...
void managementDetail::up_push() //提交槽
{
    ui->upFail->setVisible(false);
    ui->upSuccess->setVisible(false);
    ...
    Projectdb.openDb();
    Projects_books nowProject;
    Projects_books oldbook;
    oldbook = Projectdb.searchProjectInfo(nowID);
    ...
    nowProject.father = oldbook.father;
    bool flag;
...
void managementDetail::list_push(int passID)  //更新详情
{
    nowID = passID;
